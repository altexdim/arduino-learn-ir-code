 ...
 * 
 * @return int  Уровень сигнала на пине, подключенном с кнопке
 *                0  = LOW, на пине низкий уровень
 *                1  = HIGH, на пине высокий уровень
 *                -1 = неопределённый сигнал, дребезг контактов
 */
int getLearnButtonState() {
  int state = digitalRead(LEARN_BUTTON_PIN);

  for (int i = 0; i < 3; i++) {
    delay(10);
    if (digitalRead(LEARN_BUTTON_PIN) != state) {
      return -1;
    }
  }
...
void processLearnButton() {
  int state = getLearnButtonState();
  if (state == -1) { // Неопределённое состояние кнопки
    return;
  }
  if (state == learnModeEnabled) { // Состояние кнопки не поменялось
    return;
  }

  learnModeEnabled = !state; // Так как кнопка с подтягивающим резистором, то нажатие на кнопку инвертировано относительно уровня сигнала на пине
}
