...
/*
  ...
  Другим концом тактовая кнопка подключается к Vin. Так что при замыкании кнопки на этом пине будет высокий уровень.
  Так же к пину подключен стягивающий резистор 10кОм на землю.
*/
int LEARN_BUTTON_PIN = 10;
/*
...
int IR_CODE_DATA_ADDRESS = 0;

// состояние кнопки - нажата(1) или нет(0)
...
void processLearnButton() {
  ...
  if (digitalRead(LEARN_BUTTON_PIN) != learnButtonState) {
    change = 1;
    for (int i = 0; i < 3; i++) {
      delay(10);
      if (digitalRead(LEARN_BUTTON_PIN) == learnButtonState) {
  ...
  if (change) { // Если поменялось состояние кнопки (другими словами если нажали или отпустили кнопку)
    learnButtonState = !learnButtonState;
    learnModeEnabled = !learnButtonState;
  }
}
